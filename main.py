...
motor_pins_A = [17, 18]
motor_pins_B = [21, 22]
for pin in motor_pins_A + motor_pins_B:
    GPIO.setup(pin, GPIO.OUT)
    GPIO.output(pin, GPIO.LOW)

def motor_A(forward):
    GPIO.output(motor_pins_A[0], GPIO.HIGH if forward else GPIO.LOW)
    GPIO.output(motor_pins_A[1], GPIO.LOW if forward else GPIO.HIGH)

def motor_B(forward):
    GPIO.output(motor_pins_B[0], GPIO.HIGH if forward else GPIO.LOW)
    GPIO.output(motor_pins_B[1], GPIO.LOW if forward else GPIO.HIGH)
